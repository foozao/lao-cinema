...
function getRateLimitKey(type: 'login' | 'forgot-password' | 'video-token' | 'trailer-token', identifier: string): string {
  return `${type}:${identifier}`;
}

export function checkRateLimit(
  type: 'login' | 'forgot-password' | 'video-token' | 'trailer-token',
  identifier: string,
...
export function resetRateLimit(type: 'login' | 'forgot-password' | 'video-token' | 'trailer-token', identifier: string): void {
  const key = getRateLimitKey(type, identifier);
  rateLimitStore.delete(key);
}
