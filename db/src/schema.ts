...
export const videoSources = pgTable('video_sources', {
  ...
  burnedSubtitlesLanguage: text('burned_subtitles_language'), // ISO 639-1 code (e.g., 'en', 'lo', 'th')
  createdAt: timestamp('created_at').defaultNow().notNull(),
});
...
export const subtitleTracks = pgTable('subtitle_tracks', {
  ...
  linePosition: integer('line_position').default(85).notNull(), // VTT line position (0-100%, default 85 = bottom)
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
});
...
export const trailers = pgTable('trailers', {
  ...
  thumbnailUrl: text('thumbnail_url'), // Thumbnail image for video trailers
...
export const users = pgTable('users', {
  ...
  preferredSubtitleLanguage: text('preferred_subtitle_language'), // ISO 639-1 code (en, lo, th, etc.)
  alwaysShowSubtitles: boolean('always_show_subtitles').default(false).notNull(), // Show subtitles even if movie is in preferred language
